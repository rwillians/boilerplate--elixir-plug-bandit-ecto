defmodule WorkflowingWeb.Endpoint do
  @moduledoc false
  ...
  import WorkflowingWeb.Controller,
    only: [
      http_error: 1,
      send_resp_json: 2
    ]
  ...
  ## Exemplos
    ...
    ```elixir
    opts =
      Application.get_env(:workflowing, #{__MODULE__}) ||
        [port: 3000, run_server?: true]

    children =
      List.flatten([
        Workflowing.Repo,
        #{__MODULE__}.children_spec(opts)
      ])
  ...
  def children_spec(opts \\ []) do
    ...
    case run_server? do
      true -> [{Bandit, [plug: __MODULE__, scheme: :http, port: port]}]
      false -> []
    end
  end
  ...
  def get_config(opts \\ []) do
    Application.fetch_env!(:workflowing, __MODULE__)
    |> Keyword.merge(opts)
  ...
  def put_config(opts) do
    ...
    Application.put_env(:workflowing, __MODULE__, updated_config)
  end
end
